...
// table and fields
#define ndb_internal_make_table_detail_entity(r, data, i, item) BOOST_PP_COMMA_IF(BOOST_PP_NOT_EQUAL(i, 0)) BOOST_PP_CAT(BOOST_PP_VARIADIC_ELEM(0, ndb_internal_unpack(item)), _)
...
#define ndb_internal_make_field_impl_args(data, item) (data, item)

#define ndb_internal_make_field(r, data, item) BOOST_PP_EXPAND(ndb_internal_make_field_impl ndb_internal_make_field_impl_args(data, ndb_internal_unpack(item)) )

#define ndb_table(TABLE_NAME, ...) namespace ndb::tables { \
    template<class Model> struct TABLE_NAME : ::ndb::table<Model> { \
    BOOST_PP_LIST_FOR_EACH( \
        ndb_internal_make_field, \
        TABLE_NAME, \
    ...
    < \
        ::ndb::entity< \
            BOOST_PP_LIST_FOR_EACH_I( \
                ndb_internal_make_table_detail_entity, \
                BOOST_NIL, \
                BOOST_PP_VARIADIC_TO_LIST(__VA_ARGS__) \
            ) \
        > \
